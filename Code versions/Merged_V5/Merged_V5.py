...
def sequential_process_execution(chatgpt_api_key):
    ...
    while timer_running:
        ...
        if chatgpt_enabled_var.get():  # Only call ChatGPT if the toggle is checked
            ...
            print(f"Combined Output (No ChatGPT):\n{F1}\n{F2}\n{F3}")
...
def generate_twitch_style_username(chatgpt_api_key):
    prompt = (
        ...
        "Can use stream title or about to generate name"
...
# ChatGPT toggle
chatgpt_enabled_var = tk.BooleanVar(value=True)  # Default is enabled
chatgpt_enabled_checkbox = tk.Checkbutton(root, text="Enable ChatGPT", variable=chatgpt_enabled_var)
